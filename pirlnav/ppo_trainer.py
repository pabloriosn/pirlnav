...
class PIRLNavPPOTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(self, checkpoint_path: str, in_observation):
        ...
        if self.aux:
            ...
            # batch = batch_obs(self.observations, device=self.device, cache=self._obs_batching_cache)
            # batch = apply_obs_transforms_batch(batch, self.obs_transforms)  # type: ignore

            self.test_recurrent_hidden_states = torch.zeros(self.config.NUM_ENVIRONMENTS, self.actor_critic.num_recurrent_layers, policy_cfg.STATE_ENCODER.hidden_size, device=self.device,)

            self.prev_actions = torch.zeros(self.config.NUM_ENVIRONMENTS, *action_shape, device=self.device, dtype=torch.long if discrete_actions else torch.float,)
        ...
        self.observations[0]['rgb'] = in_observation['image']
        self.observations[0]['objectgoal'] = np.array([in_observation['obj_goal']])

        # Update the position
        self.observations[0]['gps'] = np.array([in_observation['position'][0], in_observation['position'][1]], dtype=np.float32)
        self.observations[0]['compass'] = np.array([in_observation['position'][2]], dtype=np.float32)
        ...
        with torch.no_grad():
            (_, actions, _, self.test_recurrent_hidden_states,) = self.actor_critic.act(batch,
